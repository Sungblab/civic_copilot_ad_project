...
| FR-009 | 문장 편집 | P0 | 사용자가 AI 문장을 수정할 수 있다. | 수정 후 복사 가능 |
| FR-010 | 결과 복사 | P0 | 생성된 문장을 클립보드에 복사할 수 있다. | 원터치 복사 |
| FR-011 | 외부 신고 안내 | P0 | 안전신문고/지자체 앱/120 등으로 이동 방법을 안내한다. | 링크 또는 안내 표시 |
...
| FR-017 | 사용자 피드백 | P1 | AI 추천이 맞았는지 사용자가 평가한다. | 좋아요/수정/신고 성공 여부 |
...
| NFR-009 | 감사성 | AI 결과, 룰 버전, 사용자 수정 이력 저장 | 분쟁/개선용 로그 |
| NFR-010 | 책임 제한 | AI 결과가 행정 처분 확정이 아님을 명확히 고지 | 고지 문구 상시 표시 |
...
| Backend API | 사용자 요청 처리, 데이터 저장, AI 호출 조율 |
...
| PostgreSQL/PostGIS | 사건, 위치, 사용자 피드백, 중복 후보 저장 |
| Object Storage | 사진/동영상 저장 |
| Audit Log | AI 판단 결과, 룰 버전, 사용자 수정 이력 저장 |
...
| PATCH | `/v1/incidents/{id}/draft` | 사용자 수정문 저장 |
...
```json
{
  ...
  "comment": "문장을 거의 그대로 사용했습니다.",
  "user_selected_type": "SIDEWALK_DAMAGE"
}
```
...
| UI-010 | 외부 신고 안내 | 앱/웹 이동, 제출 체크리스트 |
| UI-011 | 피드백 | 추천이 맞았는지, 신고 성공 여부 |
...
- 민원 유형, 지역, 피드백 등 비식별 데이터는 모델 개선에 활용
...
| 시간 | 내용 | 핵심 메시지 |
|---:|---|---|
| 0:00~0:45 | 공감 스토리 | “동네 문제를 봐도 어디에 신고할지 몰라 지나친다” |
| 0:45~1:30 | 문제 정의 | 신고 플랫폼은 있지만 신고 전 단계가 어렵다 |
| 1:30~2:20 | 시장조사 | 안전신문고/서울스마트/AI 안전신문고가 이미 있다 |
| 2:20~3:10 | 차별화 | 우리는 접수 앱이 아니라 신고 전 AI 코파일럿이다 |
| 3:10~4:20 | 시스템 설명 | 사진+위치+텍스트 → 유형/증거/채널/문장 |
| 4:20~5:20 | AI 요소기술 | VLM, LLM, RAG, 지도, 위치 DB, 중복 탐지 |
| 5:20~6:10 | 기대 효과 | 시민 편의, 행정 효율, 지역 안전 개선 |
| 6:10~7:00 | 결론 | domain-specific AI로 기존 공공서비스의 빈틈을 메운다 |
...
8. 공공데이터포털 — 국민권익위원회 민원빅데이터 분석정보 API  
   ...
   - URL: https://www.data.go.kr/data/15101903/openapi.do
